...
EXTRA_DIST = README.inetd README.slirp ChangeLog-0.8 	version reconf Makefile.uClinux config.embed.h 	CVS html samples $(man_MANS)
...
pptpd_SOURCES =  	pptpd.c configfile.c pptpmanager.c compat.c getopt.c getopt1.c 	compat.h configfile.h ctrlpacket.h defaults.h inststr.h our_getopt.h our_syslog.h ppphdlc.h pptpctrl.h pptpdefs.h pptpgre.h pptpmanager.h

pptpctrl_SOURCES =  	pptpctrl.c ctrlpacket.c inststr.c compat.c pptpgre.c ppphdlc.c
...
pptpmanager.$(OBJEXT) compat.$(OBJEXT) getopt.$(OBJEXT) \
getopt1.$(OBJEXT)
pptpd_DEPENDENCIES = 
pptpd_LDFLAGS = 
pptpctrl_OBJECTS =  pptpctrl.$(OBJEXT) ctrlpacket.$(OBJEXT) \
inststr.$(OBJEXT) compat.$(OBJEXT) pptpgre.$(OBJEXT) ppphdlc.$(OBJEXT)
pptpctrl_DEPENDENCIES = 
pptpctrl_LDFLAGS = 
...
TAR = gtar
GZIP_ENV = --best
SOURCES = $(pptpd_SOURCES) $(pptpctrl_SOURCES)
OBJECTS = $(pptpd_OBJECTS) $(pptpctrl_OBJECTS)
