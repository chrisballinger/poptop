 ...
 * Modified for PoPToP
 *
 * $Id: pptpgre.c,v 1.5 2005/07/12 06:59:29 quozl Exp $
 */
...
/*** time_now_usecs ***********************************************************/uint64_t time_now_usecs()
{
    struct timeval tv;
...
int decaps_gre(int fd, int (*cb) (int cl, void *pack, unsigned len), int cl)
{
	...
	if (PPTP_GRE_IS_S(ntoh8(header->flags))) {	/* payload present */
		...
		/* patch for reordering */
			stats.rx_accepted++;
			syslog(LOG_WARNING, "GRE: Accepting duplicate packet");
		/* end patch for reordering */
		...
		/* patch for reordering */
			syslog(LOG_WARNING, "GRE: Buffering out of order packet");
...
/* patch for reordering */
/*** dequeue_gre **************************************************************/int dequeue_gre (callback_t callback, int cl)
{
    pqueue_t *head;
    int status;
...
/* FIXME: replace with syslog()
            if (log_level >= 2)
                log("timeout waiting for %d packets", head->seq - gre.seq_recv - 1);        
*/	}
/*
if (log_level >= 2)
