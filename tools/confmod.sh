...
k_ver=$(cat /proc/sys/kernel/osrelease | cut -d "." -f1-2)
...
if [ "$k_ver" = "2.4" ]; then
	...
	TEMP=$(cat /etc/modules.conf | grep "alias char-major-108 ppp")
	if [ -z "$TEMP" ]; then echo "alias char-major-108 ppp" >> /etc/modules.conf; fi
fi
